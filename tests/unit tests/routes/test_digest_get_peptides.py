...
from tests.factories import DigestFactory, PeptideFactory, UserFactory


@pytest.mark.unit
def test_get_digest_peptides_by_id_success(
    client: TestClient, seeded_criteria: list[Criteria]
) -> None:
    """Test successfully getting peptides for a digest with all functions patched."""
    # setup
    user = UserFactory.create()
    digest = DigestFactory.create(user=user)
    peptide1 = PeptideFactory.create(digest=digest, rank=1)
    peptide2 = PeptideFactory.create(digest=digest, rank=2)
    peptides = [peptide1, peptide2]
    criteria = seeded_criteria
...
@pytest.mark.unit
def test_get_digest_peptides_digest_belongs_to_different_user(
    client: TestClient,
) -> None:
    ...
    digest = DigestFactory.create(user=other_user)
    ...
    with (
        ...
        mock_get_digest.side_effect = HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No Digest records found with user_id={user.id}, id={digest.id}.",
        )
...
@pytest.mark.unit
def test_get_digest_peptides_validation_error(
    client: TestClient,
    ...
    with (
        ...
        mock_from_peptides.side_effect = raise_validation_error
    ...
    data = response.json()
    assert (
        "formatting the response" in data["detail"].lower()
        or "error occurred" in data["detail"].lower()
    )
    mock_get_user.assert_called_once_with(ANY, id=user.id)
    mock_get_digest.assert_called_once_with(ANY, user_id=user.id, id=digest.id)
    mock_get_peptides.assert_called_once_with(ANY, digest_id=digest.id)
...
@pytest.mark.unit
def test_get_digest_peptides_attribute_error(
    client: TestClient,
    seeded_criteria: list[Criteria],
) -> None:
    ...
    user = UserFactory.build()
    digest = DigestFactory.build(user=user)
    _ = PeptideFactory.build(digest=digest)
    peptide_mock = MagicMock()
    peptide_mock.criteria = None
    ...
    with (
        ...
        mock_from_peptides.side_effect = AttributeError(
    ...
    data = response.json()
    assert (
        "processing peptide data" in data["detail"].lower()
        or "error occurred" in data["detail"].lower()
    )
    mock_get_user.assert_called_once_with(ANY, id=user.id)
    mock_get_digest.assert_called_once_with(ANY, user_id=user.id, id=digest.id)
